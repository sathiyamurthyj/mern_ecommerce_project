...
export const createCheckoutSession = async(req,res)=>{
    try {
        ...
        const lineItems  = products.map(product =>{
            const amount = Math.round(product.price*100); // stripe wants to receive in format of cents
        ...
        console.log("Error in createCheckout session controller", error.message);
...
async function createNewCoupon(userId){
    ...
    const newCoupon = new Coupon({
        ...
        expirationDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days from creation date
        userId: userId
    });
